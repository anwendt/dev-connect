...
By default, VS Code uses the normal local user profile. Existing GitHub sign-in
state, GitHub Copilot authentication, local settings, and locally installed
extensions remain available.

If `vscode.isolatedUserDataDir: true` is configured, `dev-connect` launches VS
Code with a session-specific user-data directory. That directory sets
`remote.SSH.configFile` to the temporary SSH config generated by `dev-connect`.
